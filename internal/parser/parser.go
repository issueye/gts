...
func (p *Parser) parsePattern() ast.Pattern {// Parse first primary pattern
...
func (p *Parser) parsePrimaryPattern() ast.Pattern {switch p.cur.Type {
	case lexer.TOKEN_NUMBER, lexer.TOKEN_STRING, lexer.TOKEN_TRUE, lexer.TOKEN_FALSE, lexer.TOKEN_NULL, lexer.TOKEN_UNDEFINED:
		...
		if p.curTokenIs(lexer.TOKEN_DOT_DOT) || p.curTokenIs(lexer.TOKEN_DOT_DOT_EQ) {
			...
			return &ast.RangePattern{Pos_: p.pos(), TokenLit: "..", Start: lit, End: end, Inclusive: inclusive}
		}
		return &ast.LiteralPattern{Pos_: p.pos(), TokenLit: lit.TokenLiteral(), Value: lit}
	case lexer.TOKEN_IDENT:
		if p.cur.Literal == "_" {
			tokLit := p.cur.Literal
			p.nextToken()
...
func (p *Parser) parseType() *ast.TypeAnnotation {
	...
	if p.curTokenIs(lexer.TOKEN_LBRACK) && p.peekTokenIs(lexer.TOKEN_RBRACK) {
		t.Kind = ast.TK_ARRAY
		t.ArrayOf = &ast.TypeAnnotation{Kind: ast.TK_PRIMITIVE, Name: t.Name}
		p.nextToken()
		p.nextToken()
		return t
	}
	if p.curTokenIs(lexer.TOKEN_PIPE) {
		t.Kind = ast.TK_UNION
		t.Union = []*ast.TypeAnnotation{t}
		for p.curTokenIs(lexer.TOKEN_PIPE) {
			p.nextToken()
			t.Union = append(t.Union, p.parseType())
		}
		return t
	}
